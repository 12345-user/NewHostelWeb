...
async function exchangeAuthCode(
  ...
  if (!env.kimiAuthUrl || !env.appId || !env.appSecret) {
    throw new Error(
      "Kimi OAuth is not configured. Please set APP_ID, APP_SECRET and KIMI_AUTH_URL in .env",
    );
  }
...
function getJwks() {
  if (!env.kimiAuthUrl) {
    throw new Error("Kimi OAuth is not configured. Missing KIMI_AUTH_URL in .env");
  }
  if (!jwks) {
